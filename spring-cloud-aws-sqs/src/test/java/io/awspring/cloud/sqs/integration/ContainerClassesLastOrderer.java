...
/**
 * Schedules the classes that need no container first, so they run while Localstack is still starting instead of
 * queueing behind the classes that wait for it. Ordering runs during discovery, before any test, which is also the
 * moment to ask for the container so it comes up alongside them.
 */
public class ContainerClassesLastOrderer implements ClassOrderer {
