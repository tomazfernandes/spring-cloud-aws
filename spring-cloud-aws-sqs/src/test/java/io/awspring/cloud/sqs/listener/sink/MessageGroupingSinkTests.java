...
class MessageGroupingSinkTests {
	...
	void maintainsOrderWithinEachGroup() {
		...
		List<Message<Integer>> firstMessageGroupMessages = IntStream.range(0, 10)
			.mapToObj(index -> MessageBuilder.withPayload(index).setHeader(SqsHeaders.SQS_MESSAGE_ID_HEADER, UUID.randomUUID()).setHeader(header, firstMessageGroupId).build()).collect(toList());
		List<Message<Integer>> secondMessageGroupMessages = IntStream.range(0, 10)
			.mapToObj(index -> MessageBuilder.withPayload(index).setHeader(SqsHeaders.SQS_MESSAGE_ID_HEADER, UUID.randomUUID()).setHeader(header, secondMessageGroupId).build()).collect(toList());
		List<Message<Integer>> thirdMessageGroupMessages = IntStream.range(0, 10)
			.mapToObj(index -> MessageBuilder.withPayload(index).setHeader(SqsHeaders.SQS_MESSAGE_ID_HEADER, UUID.randomUUID()).setHeader(header, thirdMessageGroupId).build()).collect(toList());
		...
		MessageGroupingSinkAdapter<Integer> sinkAdapter = new MessageGroupingSinkAdapter<>(new OrderedMessageSink<>(), message -> message.getHeaders().get(header, String.class));
